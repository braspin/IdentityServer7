...
using IdentityServer7.Stores.Serialization;
...
namespace IdentityServer7.Stores
{
    ...
    public class DefaultReferenceTokenStore : DefaultGrantStore<Token>, IReferenceTokenStore
    {
        ...
        {
            return CreateItemAsync(token, token.ClientId, token.SubjectId, token.SessionId, token.Description, (DateTime) token.CreationTime!, token.Lifetime);
        }
